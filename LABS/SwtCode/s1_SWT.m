...
figure,imshow(SwtGuiyi);
%saveas(gcf(SwtGuiyi),'2swt.png');  %在运行这条语句的是时候不是提示gcf参数错误，可以修改成下面这句话
saveas(gcf(),'2_swt.png');
%imwrite(SwtGuiyi,'2swt.png'); %可以替换上面的语句
clear LL8;
clear ll;
clear i;
clear j;
...
%% 前部分只是计算出了像素点的SWT特征值并绘图表示，并未进行区域定位，即根据SWT特征分类，是文本区域还是非文本区域
