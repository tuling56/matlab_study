%功能：倾斜矫正
%来自于：http://www.cnblogs.com/tiandsp/archive/2012/12/16/2820916.html
%说明：
%   必须有标定点，才可以从原来思辨的四个点和新矩形的四个点得到一个变换矩阵，根据这个矩阵再作用于全局图像就可以了
%   详细的原理参考上面的链接
...
img= imread('rect1.jpg');
img= rgb2gray(img);
imshow(mat2gray(img));
[M N] = size(img);

dot=ginput();       %取四个点，依次是左上，右上，左下，右下,这里我取的是书的四个角 ，然后按Enter键
