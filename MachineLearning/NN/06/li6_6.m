...
for k=1:L
    time(k)=k;
    y(k)=-a(2:na+1)*yk+b*duk(d:d+nb)+c*[xi(k);xik];           %采集输出数据
    %递推增广最小二乘法
    phie=[-yk(1:na);duk(d:d+nb);xiek];
    ...
    for j=1:N
        ...
        for i=0:nb
            if j-d-i<0
                ym(k+j)=ym(k+j)+be(i+1)*duk(d+i-j);
            end
        end
        for i=0:nc
            if j-i==0
...
figure;
subplot(2,1,1);plot(time,w(1:L),'r-.',time,y);
...
legend('a_1','a_2','a_3','b_0','b_1','c_1');
axis([0 L -4 4]);
